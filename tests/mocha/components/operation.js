...
describe('Operation', function() {
  it('monkeyset.Operation.add()', async () => {
    ...
    assert.deepEqual(monkeyset.Filter.fetch('set', 0).result(), [[1, 2, 3, 4, 5, 6]])
  })
  it('monkeyset.Operation.clear()', async () => {
    const monkeyset = new MonkeySet([1, 2, 3, 4, 5, 6])
    monkeyset.Operation.clear()
    assert.equal(monkeyset.Filter.fetch('sets').result().length, 0)
    assert.deepEqual(monkeyset.Filter.fetch('sets').result(), [])
  })
})
